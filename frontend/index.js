function moduleProject3() {
  ...
  function buildLearnerCard(learner, languages) {
    ...
    const faveP = document.createElement('p')
    const favLanguage = languages.find(lang => lang.id === learner.favLanguage)
    faveP.textContent = `Favorite Language: ${favLanguage.name}`;

    [nameP, idElement, dobP, faveP].forEach(p => {
      card.appendChild(p)
    })
 ...
 
  {
    ...
    //  ✨ do your magic here
     learners.forEach(learner => {
      const learnCard = buildLearnerCard(learner, languages)
